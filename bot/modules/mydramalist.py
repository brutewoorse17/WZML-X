...
async def mydramalist_search(_, message):
    if " " in message.text:
            ...
            async with sess.get(f"{MDL_API}/search/q/{q(title)}") as resp:
                if resp.status != 200:
                    return await editMessage(
                        temp,
                        "<i>No Results Found</i>, Try Again or Use <b>MyDramaList Link</b>",
                    )
                mdl = await resp.json()
        ...
        await editMessage(
            temp, "<b><i>Dramas found on MyDramaList :</i></b>", buttons.build_menu(1)
        )
    else:
        await sendMessage(
            message,
            f"<i>Send Movie / TV Series Name along with /{BotCommands.MyDramaListCommand} Command</i>",
        )
        ...
        if mdl.get("poster"):
            try:  # Invoke Raw Functions
                await message.reply_to_message.reply_photo(
                    mdl["poster"], caption=cap, reply_markup=buttons.build_menu(1)
                )
            ...
            await sendMessage(
                ...
                "https://te.legra.ph/file/5af8d90a479b0d11df298.jpg",
            )
