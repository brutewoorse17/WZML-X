...
async def get_user_settings(from_user, key=None, edit_type=None, edit_mode=None):
    ...
    if key is None:
        ...
        mediainfo = (
            "Enabled" if user_dict.get("mediainfo", config_dict["SHOW_MEDIAINFO"]) else "Disabled"
        )
        buttons.ibutton(
        ...
        dailytas = (
            ...
            else config_dict["DAILY_TASK_LIMIT"] or "️∞"
            if user_id != OWNER_ID
            else "∞"
        )
        ...
        mprefix = (
            "Not Exists"
            if (val := user_dict.get("mprefix", config_dict.get("MIRROR_FILENAME_PREFIX", "")))
            == ""
            else val
        )
        ...
        msuffix = (
            "Not Exists"
            if (val := user_dict.get("msuffix", config_dict.get("MIRROR_FILENAME_SUFFIX", "")))
            == ""
            else val
        )
        ...
        mremname = (
            "Not Exists"
            if (val := user_dict.get("mremname", config_dict.get("MIRROR_FILENAME_REMNAME", "")))
            == ""
            else val
        )
        ...
        lcaption = (
            "Not Exists"
            if (val := user_dict.get("lcaption", config_dict.get("LEECH_FILENAME_CAPTION", "")))
            == ""
            else val
        )
        buttons.ibutton(
        ...
        lprefix = (
            "Not Exists"
            if (val := user_dict.get("lprefix", config_dict.get("LEECH_FILENAME_PREFIX", ""))) == ""
            else val
        )
        buttons.ibutton(
        ...
        lsuffix = (
            "Not Exists"
            if (val := user_dict.get("lsuffix", config_dict.get("LEECH_FILENAME_SUFFIX", ""))) == ""
            else val
        )
        buttons.ibutton(
        ...
        lremname = (
            "Not Exists"
            if (val := user_dict.get("lremname", config_dict.get("LEECH_FILENAME_REMNAME", "")))
            == ""
            else val
        )
        buttons.ibutton(
        ...
        if key == "rcc":
            ...
            ddl_mode = (
                "Enabled"
                if key in (ddl_dict := user_dict.get("ddl_servers", {})) and ddl_dict[key][0]
                else "Disabled"
            )
            text = (
        ...
        if (
            ...
            and set_exist != "Not Exists"
            and (
                set_exist != get_readable_file_size(config_dict["LEECH_SPLIT_SIZE"]) + " (Default)"
            )
        ):
            if key == "thumb":
    ...
    if key in ["gofile", "streamtape"]:
        ...
        for td_item in value.split("\n"):
            ...
            td_details = td_item.rsplit(
                maxsplit=(
                    2
                    if split_ck[-1].startswith("http") and not is_gdrive_link(split_ck[-1])
                    else 1
            ...
            if len(td_details) > 1:
                ...
                if await sync_to_async(GoogleDriveHelper().getFolderData, td_details[1]):
                    user_tds[td_details[0]] = {
                        ...
                        "index_link": (td_details[2].rstrip("/") if len(td_details) > 2 else ""),
                    }
        value = user_tds
        return_key = "mirror"
    elif key == "ldump":
